...
import {
  ...
  Scale, Route as RouteIcon, Ticket, CalendarRange, ChevronRight, Plus, Map,
} from "lucide-react";
...
const STATUS_TONE = {
  ...
  completed: "bg-white text-ink-900/45 border border-sand",
};
...
function CurrentTripCard({ onNavigate }) {
  const { currentTrip, loadingTrip } = useCurrentTrip();
  const { t } = useLanguage();
  ...
  if (!currentTrip) {
    return (
      <Link
        to="/dashboard"
        onClick={onNavigate}
        className="group flex items-center gap-2.5 mx-1 mb-3 px-3 py-2.5 rounded-xl border border-dashed border-sand hover:border-teal/50 hover:bg-white/60 transition-colors"
      >
  ...
  return (
    <Link
      to="/itinerary"
      onClick={onNavigate}
      className="group block mx-1 mb-3 px-3 py-2.5 rounded-xl bg-white border border-sand hover:border-teal/50 hover:shadow-soft transition-all"
    >
                  ...
                  className={({ isActive }) =>
                    `group relative flex items-center gap-3 px-4 py-1.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                      isActive
                        ...
                        : muted
                          ? "text-ink-900/30 hover:bg-white/60 hover:text-ink-900/60"
...
function UserFooter({ user, onLogout, onNavigate }) {
  ...
  return (
    <div className="flex flex-col gap-1 pt-2.5 mt-2.5 border-t border-sand shrink-0">
      ...
      <div className="flex items-center gap-3 px-3 py-2.5 mt-1 bg-white/70 border border-sand rounded-xl">
