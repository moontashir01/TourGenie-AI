  ...
  return (
    <div className={`flex items-end justify-between gap-4 mb-5 ${className}`}>
      <div className="min-w-0">
        <h2 className="font-display text-lg text-ink-900 flex items-baseline gap-2">
          {title}
          {/* The count is information, not decoration — it goes quiet next to
              the heading rather than becoming a second badge. */}
          {count !== undefined && <span className="text-sm font-body text-ink-900/35 tabular-nums">{count}</span>}
        </h2>
        {description && <p className="text-sm text-ink-900/55 mt-0.5">{description}</p>}
      </div>
