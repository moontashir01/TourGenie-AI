                    ...
                    <ul className="space-y-4">
                      {dayItems.map((item) => (
                        ...
                        <li
                          ref={ref}
                          style={style}
                          className={`flex gap-2 group/row rounded-xl ${isDragging ? "relative z-10" : ""}`}
                        >
                          {/* Handle rather than whole-row dragging: the row
                              contains its own buttons (transport picking). */}
                          <button
                            {...handleProps}
                            className="shrink-0 self-start mt-0.5 w-5 h-6 flex items-center justify-center rounded text-ink-900/20 opacity-0 group-hover/row:opacity-100 focus-visible:opacity-100 hover:text-teal-dark hover:bg-teal-light/50 cursor-grab active:cursor-grabbing transition-all touch-none"
