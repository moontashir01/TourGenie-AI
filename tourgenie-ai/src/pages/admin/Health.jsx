  ...
  return (
    <div className="space-y-8">
      ...
      <section className="card p-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1.5 max-h-72 overflow-y-auto pr-2">
          {(db.collections || []).map((collection) => (
            <div key={collection.name} className="flex items-baseline justify-between gap-2 text-sm border-b border-sand/60 py-1">
              <span className="font-mono text-xs text-ink-600 truncate">{collection.name}</span>
        ...
        <p className="text-xs text-ink-500 mt-4 tabular-nums">
          {(db.total_documents || 0).toLocaleString()} documents across {(db.collections || []).length} collections
        </p>
      </section>

      <section className="card p-6">
        <h3 className="font-display text-xl text-ink-900 mb-1">External providers</h3>
        <p className="text-sm text-ink-600 mb-5">
            ...
            <tbody className="divide-y divide-sand">
              {(health?.providers || []).map((provider) => (
                <tr key={provider.key}>
                  ...
                  <td className="py-3 text-ink-600 text-xs">{provider.purpose}</td>
                  <td className="py-3">
                    {provider.configured ? (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <section className="card p-6">
          <h3 className="font-display text-xl text-ink-900 mb-4 flex items-center gap-2">
      ...
      {health?.generated_at && (
        <p className="text-xs text-ink-500">Checked {new Date(health.generated_at).toLocaleString()}</p>
      )}
    </div>
  );
}
...
function LastCall({ call, configured }) {
  if (!configured) return <span className="text-ink-500">—</span>;
  if (!call) return <span className="text-ink-500">not called yet</span>;
  return (
    <span className={`inline-flex items-start gap-1.5 ${call.ok ? "text-teal-dark" : "text-sunset-dark"}`}>
      ...
      <span>
        ...
        {call.detail && <span className="block text-ink-500 max-w-xs">{call.detail}</span>}
      </span>
    </span>
  );
}
